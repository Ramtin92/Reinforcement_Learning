...
class Testbed:
    ...
    def __init__(self, number_arms, mean, std):
        # Number of arms
...
if __name__ == "__main__":
    ...
    for flag in modes:
        ...
        testbed = Testbed(number_arms=number_arms, mean=0, std=0.01)
        agents = [Agent(number_arms=number_arms, epsilon_prob=0.1)]
        environment = Environment(testbed=testbed, agents=agents, steps=steps, iterations=iterations, flag=flag)

        # Run Environment
        print("Running...")
        scores_one, scores_two = environment.play
        scores_one_list.append(scores_one)
        scores_two_list.append(scores_two)
        print("Execution time: %s seconds" % (time.time() - start_time))

    print_plots(modes, scores_one_list, scores_two_list)
